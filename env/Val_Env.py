...
class Val_Env(gym.Env):
   
    metadata = {'render.modes': ['human']}

    def __init__(self, df, day = 0, turbulence_threshold=150, iteration=''):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            plt.savefig('results/portfolio_value_validation_{}.png'.format(self.iteration))
            plt.close()
            
            step_total_value = pd.DataFrame(self.asset_memory)
            step_total_value.to_csv('results/portfolio_value_validation_{}.csv'.format(self.iteration))
            ...
            sharpe = (4**0.5)*step_total_value['daily_return'].mean()/ \
                  step_total_value['daily_return'].std()
